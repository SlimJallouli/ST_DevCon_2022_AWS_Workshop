...
int32_t BSP_MOTION_SENSOR_EnableFreeFallDetection(uint32_t Instance, uint32_t Function, int32_t IRQPin)
{
	int32_t status = BSP_ERROR_NONE;
	uint8_t index;
	...
	{
		index = (Function == MOTION_GYRO) ? GYRO_ID : ((Function == MOTION_ACCELERO) ? ACCELERO_ID : MAGNETO_ID);
		if ((index == MAGNETO_ID) || (index == GYRO_ID))
		{
			/* Feature not supported for Magneto and Gyro */
			status = BSP_ERROR_FEATURE_NOT_SUPPORTED;
		}
		else
		{
			if (Motion_Sensor_FuncDrv[Instance][index]->EnableFreeFallDetection(Motion_Sensor_CompObj[Instance], IRQPin) < 0)
			{
				status = BSP_ERROR_COMPONENT_FAILURE;
			}
		}
...
int32_t BSP_MOTION_SENSOR_DisableFreeFallDetection(uint32_t Instance, uint32_t Function)
{
	int32_t status = BSP_ERROR_NONE;
	uint8_t index;
	...
	else
	{
		...
		{
			if (Motion_Sensor_FuncDrv[Instance][index]->DisableFreeFallDetection(Motion_Sensor_CompObj[Instance]) < 0)
			{
				status = BSP_ERROR_COMPONENT_FAILURE;
			}
		}
...
int32_t BSP_MOTION_SENSOR_SetFreeFallDetectionThreshold(uint32_t Instance, uint32_t Function, uint8_t Threshold)
{
	int32_t status = BSP_ERROR_NONE;
	uint8_t index;
	...
	else
	{
		...
		{
			if (Motion_Sensor_FuncDrv[Instance][index]->SetFreeFallThreshold(Motion_Sensor_CompObj[Instance], Threshold) < 0)
			{
				status = BSP_ERROR_COMPONENT_FAILURE;
			}
		}
...
int32_t BSP_MOTION_SENSOR_SetFreeFallDetectionDuration(uint32_t Instance, uint32_t Function, uint8_t Duration)
{
	int32_t status = BSP_ERROR_NONE;
	uint8_t index;
	...
	else
	{
		...
		{
			if (Motion_Sensor_FuncDrv[Instance][index]->SetFreeFallDuration(Motion_Sensor_CompObj[Instance], Duration) < 0)
			{
				status = BSP_ERROR_COMPONENT_FAILURE;
			}
		}
...
int32_t BSP_MOTION_SENSOR_GetEventStatus(uint32_t Instance, uint32_t Function, void *Status)
{
	int32_t status = BSP_ERROR_NONE;
	uint8_t index;
	...
	else
	{
		...
		{
			if (Motion_Sensor_FuncDrv[Instance][index]->GetEventStatus(Motion_Sensor_CompObj[Instance], Status) < 0)
			{
				status = BSP_ERROR_COMPONENT_FAILURE;
			}
		}
